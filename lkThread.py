...
## 쓰레드 분리 (async()?), 그래도 렉걸리면 mediapipe 제거, 그리드 제거 고려, (업피라미드, 다운 피라미드 동시사용 고려)
## vec -tiemstamp별별 x,y,속도,각도,낙상여부
...
def showFrame():
    ...
    while True:
        ...
        # Select good points
        if p1 is not None:
            good_new = p1.reshape(-1,1,2)[((st==1) & (err<8))]
            good_old = p0.reshape(-1,1,2)[((st==1) & (err<8))]
...
threadFrame = threading.Thread(target=calcVec)
threadCalc = threading.Thread(target=showFrame)
